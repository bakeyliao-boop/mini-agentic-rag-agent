...
## P-006：回答内容超出引用范围（与 glob 改造无关）

### 当前问题

Prompt-V1.5 文件名冒烟要求说明《智慧农场》的资源类型。Agent 正确回答
`video_transcript`，但还补充了年级、教材版本、单元、课次和课程内容等信息。

本次唯一 citation 是文件第 6 行：

```text
resource_type: video_transcript
```

这条引用只能证明资源类型，不能证明回答中的其他扩展信息。因此，回答虽然使用了真实引用，
但部分内容超出了引用能够支持的范围。

该问题发生在 `glob` 已正确定位并且 `read` 成功之后，属于回答与引用的语义一致性问题，
不属于 glob 路径发现问题。

### 原因边界

证据闸门已经能验证 evidence ID、路径、行号和原文未被篡改，但还不能自动判断回答中的
每一项事实是否都被所选引用支持。

### 后续事项

- 先约束回答只覆盖用户问题所需内容，减少无关扩展。
- 在 Agentic 评分中增加“回答事实是否被 citation 覆盖”的独立检查。
