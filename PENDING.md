...
证据闸门已经能验证 evidence ID、路径、行号和原文，但还不能自动判断回答中的
每一句话是否都被引用原文支持。真实冒烟中出现了轻微扩大解释。

### 后续事项

先通过 Prompt 约束回答只能使用引用原文；正式评测阶段再统计回答忠实性，必要时
增加独立的 claim-evidence 校验。

状态：证据真实性已保证，语义忠实性待评测。
