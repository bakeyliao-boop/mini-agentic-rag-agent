...
def build_knowledge_path_snapshot(
    knowledge_root: Path,
) -> list[dict[str, str]]:
    """扫描知识库一次，生成稳定排序的完整虚拟路径快照。"""

    if not isinstance(knowledge_root, Path):
        raise TypeError("knowledge root must be a Path")
    if not knowledge_root.exists():
        raise FileNotFoundError(
            f"knowledge directory does not exist: {knowledge_root}"
        )
    if not knowledge_root.is_dir():
        raise NotADirectoryError(
            f"knowledge path is not a directory: {knowledge_root}"
        )

    snapshot = [{"path": "/", "type": "directory"}]
    source_paths = sorted(
        knowledge_root.rglob("*"),
...
def glob_knowledge_snapshot_paths(
    virtual_path: str,
    pattern: str,
    snapshot: list[dict[str, str]],
) -> list[str]:
    """在内存路径快照中递归匹配文件，并返回完整虚拟路径。"""

    normalized_path = normalize_virtual_path(virtual_path)
    if not isinstance(pattern, str) or not pattern.strip():
        raise ValueError("glob pattern must not be empty")
    if not isinstance(snapshot, list):
        raise TypeError("knowledge path snapshot must be a list")
    ...
    if scope_entry.get("type") != "directory":
        raise NotADirectoryError(
    ...
    for entry in snapshot:
        ...
        normalized_entry_path = normalize_virtual_path(entry_path)
        if normalized_path == "/":
            relative_path = normalized_entry_path[1:]
        elif normalized_entry_path.startswith(path_prefix):
            relative_path = normalized_entry_path[len(path_prefix) :]
        else:
            continue
        ...
        if matches_pattern:
            matched_paths.append(normalized_entry_path)

    return sorted(matched_paths)


def list_knowledge_snapshot_entries(
    virtual_path: str,
    snapshot: list[dict[str, str]],
) -> list[dict[str, str]]:
    """从内存路径快照列出指定目录的直接子项。"""
    ...
    path_prefix = "/" if normalized_path == "/" else f"{normalized_path}/"
    entries: list[dict[str, str]] = []

    for entry in snapshot:
        entry_path = entry.get("path")
        entry_type = entry.get("type")
        if not isinstance(entry_path, str) or entry_type not in {
            "directory",
            "file",
        }:
            continue
        if entry_path == normalized_path or not entry_path.startswith(
            path_prefix
        ):
            continue

        relative_path = entry_path[len(path_prefix) :]
